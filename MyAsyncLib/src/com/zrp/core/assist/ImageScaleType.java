...
public enum ImageScaleType {
	...
	IN_SAMPLE_INT,
	/**
	 * Image will scale-down exactly to target size(scaled width or height or both will be equal to target size;
	 * depends on {@linkplain android.widget.ImageView.ScaleType ImageView's scale type}). Use it if memory economy is critically important. <br />}
	 * <b>Note:<b> If original image size is smaller than target size then original image <b>won't</b> be scaled.<br />
	 * <b>Note:</b> For creating result Bitmap (of exact size) additional Bitmap will be created with {@link android.graphics.Bitmap#createBitmap(android.graphics.Bitmap,int,int,int,int,abdroid.graphics.Matrix,boolean)}
